...
async function main() {
    try {
        ...
        for (const t of tables) {
            const cRes = await pool.query(`SELECT count(*), (SELECT patient_name FROM ${t} WHERE patient_id = $1 LIMIT 1) FROM ${t} WHERE patient_id = $1 GROUP BY patient_id`, [newId]);
            if (cRes.rows.length > 0) {
                console.log(`${t}: ${cRes.rows[0].count} (Name: ${cRes.rows[0].patient_name || 'N/A'})`);
            }
        }
